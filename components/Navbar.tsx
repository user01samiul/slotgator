  ...
  const categories = {
    integrations: {
      ...
      subcategories: [
        {
          name: 'Game providers',
          description: 'Integrate casino games from leading developers with our API.',
          href: '/game-providers',
          icon: '🎮'
        },
        {
          name: 'Slots',
          description: 'Access thousands of slot games from trusted developers.',
          ...
          <div className="hidden lg:flex items-center space-x-8">
            {Object.entries(categories).map(([key, category]) => (
              <div key={key} className="relative">
                ...
                {activeDropdown === key && (
                  <div className={`absolute z-20 left-1/2 transform -translate-x-1/2 mt-2 rounded-lg shadow-lg bg-white ring-1 ring-gray-200 overflow-hidden animate-fadeIn ${
                    key === 'products' ? 'w-[640px]' : 'w-80'
                  }`}>
                    <div className={`p-4 ${key === 'products' ? 'grid grid-cols-2 gap-x-4 gap-y-2' : ''}`}>
                        ...
                        >
                          ...
                          <div>
                            <p className={`font-medium ${
                              isActive(subcategory.href) ? 'text-[#FF2432]' : 'text-gray-900'
                            }`}>
                              {subcategory.name}
                            </p>
